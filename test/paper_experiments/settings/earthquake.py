"""
Experiment-specific factories for the EasyTPP 'earthquake' dataset.
"""
...
@register_experiment('earthquake')
# For register to work, import in trainingmanager.py
def register_earthquake_factories():
    def data_earthquake(cfg):
        return EarthquakeDataModule(
            batch_size=cfg['parameter_sets'].get('batch_size', None),
        )

    def model_earthquake(cfg, data, period_plot_val, datamodel_path, logger_custom, checkpoint=None):
        return get_model(
    ...
    def name_earthquake(time_max, cfg, custom_file_name=None):
    ...
    return {
        "data_factory": data_earthquake,
        "model_factory": model_earthquake,
        "model_namer": name_earthquake,
