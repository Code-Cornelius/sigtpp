"""
Shared model-factory logic for all EDITPP datasets.

Each dataset settings file imports make_editpp_factories() and calls it with its
own data-module class and canonical experiment name.
"""
...
def _build_model(
    ...
    if version == Architectures.SIGTPP:
        model_kwargs = {
            ...
            # sigwgan-only train-time length cap; None (default) for every other dataset.
...
def make_editpp_factories(experiment_name: str, data_module_class):
    """
    Register and return the 4-callable bundle for one EDITPP dataset.

    Parameters
    ----------
    experiment_name : str
        The name used both in @register_experiment and in model file naming.
    data_module_class : type
        The EditTPPDataModule subclass for this dataset.
    """
    ...
    def _register():
        ...
        def model_fn(cfg, data, period_plot_val, datamodel_path, logger_custom, checkpoint=None):
            return _build_model(
                ...
                # Per-dataset, sigwgan-only cap supplied at the top level of the dataset's
                # experiment.yaml (parameter_sets cannot carry it: the shared model config
                # wins the shallow merge in load_experiment_config). None => uncapped.
                train_seq_cap=cfg.get("train_seq_cap"),
            )

        def name_fn(time_max, cfg, custom_file_name=None):
