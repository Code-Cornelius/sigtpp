"""
Experiment-specific factories for the EasyTPP 'stackoverflow' dataset.
"""
...
@register_experiment('stackoverflow')
# For register to work, import in trainingmanager.py
def register_stackoverflow_factories():
    def data_stackoverflow(cfg):
        return StackOverflowDataModule(
            batch_size=cfg['parameter_sets'].get('batch_size', None),
            zero_marks=cfg['zero_marks'],
        )

    def model_stackoverflow(cfg, data, period_plot_val, datamodel_path, logger_custom, checkpoint=None):
        return get_model(
    ...
    def name_stackoverflow(time_max, cfg, custom_file_name=None):
    ...
    return {
        "data_factory": data_stackoverflow,
        "model_factory": model_stackoverflow,
        "model_namer": name_stackoverflow,
