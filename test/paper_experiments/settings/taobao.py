"""
Experiment-specific factories for the EasyTPP 'taobao' dataset.
"""
...
@register_experiment('taobao')
# For register to work, import in trainingmanager.py
def register_taobao_factories():
    def data_taobao(cfg):
        return TaobaoDataModule(
            batch_size=cfg['parameter_sets'].get('batch_size', None),
            zero_marks=cfg['zero_marks'],
        )

    def model_taobao(cfg, data, period_plot_val, datamodel_path, logger_custom, checkpoint=None):
        return get_model(
    ...
    def name_taobao(time_max, cfg, custom_file_name=None):
    ...
    return {
        "data_factory": data_taobao,
        "model_factory": model_taobao,
        "model_namer": name_taobao,
